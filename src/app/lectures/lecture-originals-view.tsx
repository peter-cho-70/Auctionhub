...
export function LectureOriginalsView() {
  ...
  const groupedDocs = useMemo(() => {
    const groups = new Map<string, typeof LECTURE_ORIGINAL_DOCS>();
    for (const doc of LECTURE_ORIGINAL_DOCS) {
      const list = groups.get(doc.groupTitle) ?? [];
      list.push(doc);
      groups.set(doc.groupTitle, list);
    }
    ...
    <div className="flex flex-col gap-6 lg:flex-row-reverse lg:items-start">
      ...
      >
        ...
        <div className="space-y-4">
          {groupedDocs.map(([groupTitle, docs]) => (
